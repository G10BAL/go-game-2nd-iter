...
public class ClientApplication extends Application {
    ...
    private StringBuilder boardBuffer = new StringBuilder(); // Buffer for accumulating board update lines from server
    ...
    public void start(Stage primaryStage) {
        ...
        javafx.scene.control.Label waitingLabel = new javafx.scene.control.Label("Connecting to server...");
    ...
    private void handleServerMessage(String message) {
        Platform.runLater(() -> {
            ...
            if (message.trim().isEmpty()) {
                ...
                if (boardBuffer.length() > 0) {
                    if (!gameStarted) {
                        ...
                        if (controller == null) {
                            System.out.println("[ClientApp] Creating display at board update because controller is null");
                            createGameDisplay();
                        } else {
                            System.out.println("[ClientApp] Not creating display at board update, controller already exists with size: " + boardSize);
                        }
            ...
            if (trimmed.startsWith("CONNECTED ") || trimmed.startsWith("COLOR ") ||
                trimmed.startsWith("BOARDSIZE ") || trimmed.startsWith("EVENT ") ||
                trimmed.startsWith("ERROR ") || trimmed.startsWith("CAPTURED ") ||
                trimmed.startsWith("TURN ") || trimmed.startsWith("RESULT ")) {
                ...
                switch (command) {
                    ...
                    case "BOARDSIZE":
                        try {
                            ...
                            System.out.println("[ClientApp] Received BOARDSIZE: " + newBoardSize + " (was: " + boardSize + ")");
                            ...
                            if (controller == null) {
                                ...
                                System.out.println("[ClientApp] Not creating display, controller already exists");
                            }
                        } catch (NumberFormatException e) {
                    ...
                    case "RESULT":
                        try {
                            ...
                            if (parts.length >= 4) {
                                ...
                                System.out.println("[ClientApp] Game result: " + resultWinner + 
                                    " won (Black: " + resultBlackScore + ", White: " + resultWhiteScore + 
                                    ", Margin: " + resultMargin + ")");
                            }
                        } catch (Exception e) {
        ...
        if (eventData.contains("GAME_STARTED")) {
            ...
            if (resultWinner != null) {
                ...
                content = String.format("%s\n\nResult:\n%s %s wins\n\nScore:\nBlack: %.1f\nWhite: %.1f\nMargin: %.1f",
                    content, winnerEmoji, resultWinner, resultBlackScore, resultWhiteScore, resultMargin);
            }
    ...
    public void sendMoveToServer(int x, int y) {
        if (client == null || !client.isConnected() || myColor == null || !gameStarted) {
            System.err.println("[ClientApp] Cannot send move: invalid state (check connection or color assignment)");
            Alert alert = new Alert(Alert.AlertType.WARNING);
            alert.setTitle("Move Error");
            alert.setContentText("Cannot make move: invalid state. Check if game started and color assigned.");
            alert.showAndWait();
            return;
        }
        // Send move 
    ...
    /**
     * Parses the board state text from the server and updates the local board.
     * Uses setColor to directly sync the state, handling captures by setting EMPTY where needed.
            ...
            if (rowMatcher.find()) {
                ...
                for (int j = 0; j < stones.length; j++) {
                    ...
                    try {
                        ...
                        System.err.println("[ClientApp] Error setting stone at (" + j + "," + rowNum + "): " + e.getMessage());
                    }
                }
            }
        }
