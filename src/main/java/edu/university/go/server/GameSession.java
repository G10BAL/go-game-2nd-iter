...
class GameSession implements GameObserver {
    ...
    void handleMove(Move move) {
        try {
            ...
            System.out.println("[GameSession] After move - BLACK captured: " + capturedByBlack + ", WHITE captured: " + capturedByWhite);
    ...
    broadcast("EVENT " + event);
        ...
        if (event == GameEvent.GAME_STARTED || event == GameEvent.MOVE_PLAYED) {
            System.out.println("[GameSession] Broadcasting game state - CAPTURED: " + capturedByBlack + " " + capturedByWhite + ", TURN: " + game.getCurrentTurn());
            
            // Cur turn and captured stones first
            broadcast("CAPTURED " + capturedByBlack + " " + capturedByWhite);
            broadcast("TURN " + game.getCurrentTurn());
            
            // Then send the board state to each player
            ...
            GameResult result = scoreCalculator.calculateScore(game.getBoard(), capturedByBlack, capturedByWhite);
            System.out.println("[GameSession] Final result: " + result);
            
            System.out.println("[GameSession] Broadcasting final state...");
            broadcast("CAPTURED " + capturedByBlack + " " + capturedByWhite);
            broadcast("TURN " + game.getCurrentTurn());
            
            String scoreMsg = String.format("RESULT %s %.1f %.1f %.1f",
                result.winner(),
                result.blackScore(),
                result.whiteScore(),
                result.margin());
